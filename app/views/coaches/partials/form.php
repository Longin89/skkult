...
<form action="<?= $this->formAction ?>" method="POST" enctype="multipart/form-data">
    ...
    <?= FH::inputTextarea('Описание', 'description', $this->coach->description, ['class' => 'form-control input-sm tiny', 'rows' => '8'], ['class' => 'form-group col-md-5 mx-auto my-4']); ?>
    ...
    <input type="file" name="images" id="coachImages" class="upload__image">
    <?php echo '</div>'; ?>
    <?php
    if (isset($this->coach->id)) {
        $this->partial('coaches', 'editImages');
    }
    ?>
